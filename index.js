...
async function run() {
    try {
        ...
        const useMasterBranch = requestedVersion == 'master' ||
            requestedVersion == 'dev' || requestedVersion == 'latest' ||
        ...
        const installDeps = core.getInput('install-deps') == undefined ||
        ...
        const BUILD_NEW_PYTHON_SUPPORT =
            core.getInput('BUILD_NEW_PYTHON_SUPPORT');
        const WITH_V4L = core.getInput('WITH_V4L');
        const ENABLE_PRECOMPILED_HEADERS =
            core.getInput('ENABLE_PRECOMPILED_HEADERS');
        const INSTALL_C_EXAMPLES = core.getInput('INSTALL_C_EXAMPLES');
        const INSTALL_PYTHON_EXAMPLES =
            core.getInput('INSTALL_PYTHON_EXAMPLES');
        ...
        if (installDeps) {
            ...
            await exec.exec(
                ...
                'libtbb2 libtiff-dev ' +
                'python-dev-is-python3 python-numpy ' +
                'x264 v4l-utils ');
            core.endGroup();
        }
        ...
        await exec.exec(
            `git clone https://github.com/opencv/opencv.git --branch ${
                version} --depth 1`);

        if (extraModules) {
            await exec.exec(
                `git clone https://github.com/opencv/opencv_contrib.git --branch ${
                    version} --depth 1`);
        }
        core.endGroup();
        ...
        const cmakeCmd = 'cmake -S opencv -B opencv/build ' +
            ...
            ' -D WITH_TBB=' + WITH_TBB + ' -D WITH_IPP=' + WITH_IPP +
            ...
            ' -D BUILD_EXAMPLES=' + BUILD_EXAMPLES + ' -D WITH_QT=' + WITH_QT +
